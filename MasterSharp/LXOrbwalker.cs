...
namespace MasterSharp
{
    public class LxOrbwalker
    {
        ...
        public static void AddToMenu(Menu menu)
        {
            ...
            var menuDrawing = new Menu("Hiển Thị", "lxOrbwalker_Draw");
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_AARange", "Tầm đánh ").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_AARange_Enemy", "Tầm đánh của địch").SetValue(new Circle(true, Color.Pink)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_Holdzone", "Giữ vị trí").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_MinionHPBar", "Thanh máu lính").SetValue(new Circle(true, Color.Black)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_MinionHPBar_thickness", "^ Chế độ hiển thị").SetValue(new Slider(1, 1, 3)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_hitbox", "Vòng tròn lính").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_Lasthit", "Lính LastHit").SetValue(new Circle(true, Color.Lime)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_nearKill", "Lính gần chết").SetValue(new Circle(true, Color.Gold)));
            menu.AddSubMenu(menuDrawing);

            var menuMisc = new Menu("Misc", "lxOrbwalker_Misc");
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_Holdzone", "Giữ vị trí").SetValue(new Slider(50, 100, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_Farmdelay", "Độ trể farm").SetValue(new Slider(0, 200, 0)));
            menuMisc.AddItem(
                new MenuItem("lxOrbwalker_Misc_ExtraWindUp", "Đỗ trể khi AA và di chuyển").SetValue(new Slider(80, 200, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AutoWindUp", "Tự động chỉnh windump").SetValue(false));
            menuMisc.AddItem(
                new MenuItem("lxOrbwalker_Misc_Priority_Unit", "Chế độ ưu tiên").SetValue(
                    new StringList(new[] {"Minion", "Hero"})));
            menuMisc.AddItem(
                new MenuItem("lxOrbwalker_Misc_Humanizer", "Humanizer Delay").SetValue(new Slider(50, 100, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AllMovementDisabled", "Tắt tât cả các di chuyển").SetValue(false));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AllAttackDisabled", "Tắt tự động đánh").SetValue(false));

            menu.AddSubMenu(menuMisc);

            var menuMelee = new Menu("Melee", "lxOrbwalker_Melee");
            menuMelee.AddItem(new MenuItem("lxOrbwalker_Melee_Prediction", "Đoán hướng di chuyển").SetValue(false));
            menu.AddSubMenu(menuMelee);

            var menuModes = new Menu("Orbwalk Mode", "lxOrbwalker_Modes");
            {
                var modeCombo = new Menu("Chiến ", "lxOrbwalker_Modes_Combo");
                modeCombo.AddItem(new MenuItem("Combo_Key", "Phím").SetValue(new KeyBind(32, KeyBindType.Press)));
                modeCombo.AddItem(new MenuItem("Combo_move", "Di chuyển").SetValue(true));
                modeCombo.AddItem(new MenuItem("Combo_attack", "Tấn công").SetValue(true));
                modeCombo.AddItem(new MenuItem("Move_target", "Di chuyển đến mục tiêu").SetValue(true));
                menuModes.AddSubMenu(modeCombo);

                var modeHarass = new Menu("Cấu rỉa", "lxOrbwalker_Modes_Harass");
                modeHarass.AddItem(
                    new MenuItem("Harass_Key", "Phím").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
                modeHarass.AddItem(new MenuItem("Harass_move", "Di chuyển").SetValue(true));
                modeHarass.AddItem(new MenuItem("Harass_attack", "Tấn công").SetValue(true));
                modeHarass.AddItem(new MenuItem("Harass_Lasthit", " Ưu tiên lasthit lính").SetValue(true));
                menuModes.AddSubMenu(modeHarass);

                var modeLaneClear = new Menu("Đẩy đường", "lxOrbwalker_Modes_LaneClear");
                ...
                modeLaneClear.AddItem(new MenuItem("LaneClear_attack", "Tấn công").SetValue(true));
                menuModes.AddSubMenu(modeLaneClear);

                var modeLaneFreeze = new Menu("Đóng băng lính", "lxOrbwalker_Modes_LaneFreeze");
                modeLaneFreeze.AddItem(
                    new MenuItem("LaneFreeze_Key", "Phím").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                modeLaneFreeze.AddItem(new MenuItem("LaneFreeze_move", "Di chuyển").SetValue(true));
                modeLaneFreeze.AddItem(new MenuItem("LaneFreeze_attack", "tấn công").SetValue(true));
                ...
                modeLasthit.AddItem(new MenuItem("LastHit_attack", "Tấn Công").SetValue(true));
                menuModes.AddSubMenu(modeLasthit);

                var modeFlee = new Menu("Chạy trốn", "lxOrbwalker_Modes_Flee");
                modeFlee.AddItem(
                    new MenuItem("Flee_Key", "Phím").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
