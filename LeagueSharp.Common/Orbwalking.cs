    ...
    public static class Orbwalking
    {
        ...
        public class Orbwalker
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                /* Misc options */
                var misc = new Menu("Mở rộng", "Misc");
                misc.AddItem(
                    new MenuItem("HoldPosRadius", "Giữ vị trí an toàn").SetShared().SetValue(new Slider(0, 0, 250)));
                misc.AddItem(new MenuItem("PriorizeFarm", "Ưu tiên farm trong cấu rỉa").SetShared().SetValue(true));
                misc.AddItem(new MenuItem("AttackWards", "Tự động phá mắt").SetShared().SetValue(false));
                misc.AddItem(new MenuItem("AttackPetsnTraps", "Tự động đánh pet và phá bẫy").SetShared().SetValue(true));
                misc.AddItem(new MenuItem("Smallminionsprio", "Dọn rừng luôn giết quái nhỏ trước").SetShared().SetValue(false));

                _config.AddSubMenu(misc);

                /* Missile check */
                _config.AddItem(new MenuItem("MissileCheck", "Dùng kiểm tra trong lúc đánh").SetShared().SetValue(true));

                /* Delay sliders */
                _config.AddItem(
                    new MenuItem("ExtraWindup", "Mỏ rộng độ trể giữa AA và di chuyển ").SetShared().SetValue(new Slider(80, 0, 200)));
                _config.AddItem(new MenuItem("FarmDelay", "Độ trể khi Farm").SetShared().SetValue(new Slider(30, 0, 200)));

                /*Load the menu*/
                _config.AddItem(
                    new MenuItem("LastHit", "Last hit").SetShared().SetValue(new KeyBind('X', KeyBindType.Press)));

                _config.AddItem(new MenuItem("Farm", "Cấu rỉa").SetShared().SetValue(new KeyBind('V', KeyBindType.Press)));

                _config.AddItem(
                    new MenuItem("LaneClear", "Đẩy đường").SetShared().SetValue(new KeyBind('V', KeyBindType.Press)));

                _config.AddItem(
                    new MenuItem("Orbwalk", "Chiến").SetShared().SetValue(new KeyBind(32, KeyBindType.Press)));
